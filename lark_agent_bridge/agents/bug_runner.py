    ...
    def _run_source_stage_pydantic_ai(
        ...
        *,
        ...
        system_prompt = (
            ...
            "## 可用工具\n"
            "### 文件读取\n"
            "- read_file(path, start_line?, end_line?): 读取文件内容，支持行范围分页（对大文件先用 get_file_outline）\n"
            "- get_file_outline(path): 获取文件符号大纲（类/函数/方法列表+行号），无需读全文\n"
            "- list_dir(path): 列出目录内容\n"
            "### 搜索\n"
            "- grep(pattern, glob_filter?, context_lines?): 正则搜索，context_lines>0 显示上下文\n"
            "- glob(pattern): 按 glob 查找文件路径，如 '**/*.kt'\n"
            "### Git 历史\n"
            "- git_log(path?, max_count?): 查看文件/目录的 git 提交历史\n"
            "- git_blame_range(path, start_line, end_line): 查看代码行的修改时间和作者\n"
            "- repo_overview(path?): 仓库概览：分支、最近提交、目录结构、技术栈\n"
            "### 语义代码搜索（有索引时可用）\n"
            ...
            "## 工作要求\n"
            "1. 复杂分析前先用 think() 写下分析计划和假设\n"
            "2. 优先用 search_codegraph/get_code_context 快速定位符号（有索引时）\n"
            "3. 对大文件先用 get_file_outline 了解结构，再用 read_file(start_line, end_line) 精读\n"
            "4. 用 grep(pattern, context_lines=5) 搜索关键词并查看上下文\n"
            "5. 用 git_log 确认代码改动时间线，用 git_blame_range 定位谁引入了问题代码\n"
